...
This chapter is aimed at delineating theories that underpin the artifact. These theories are referred to as \emph{design theories} in design science research \cite{Rose}. These theories form the basis of understanding the underlying pircinples that govern the design of Terramycelium.
...
The metrics not only demonstrate the stability and efficiency of our BD infrastructure, but also offer empirical evidence that, in an event-driven asynchronous system like Terramycelium, the focus is not solely on the amount of data processed, but rather on the effectiveness and reliability with which it is managed. This observation indicates possible consequences for scalability considerations in future implementations of similar structures.
...
The experts' perspectives on the Terramycelium architecture's applicability, strengths, and challenges are succinctly outlined in the table reftab:responses, which aggregates their responses.

\begin{table}[h]
  \centering
  \caption{Mapping of expert responses against themes.}
  \begin{tabular}{@{}llp{0.3\columnwidth}p{0.3\columnwidth}@{}}
  \hline
