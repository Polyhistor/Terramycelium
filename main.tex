...
Once the ingress controller is established, the ingress resources are configured to direct traffic to Kafka-rest-proxy. The Kafka-rest-proxy is included in the artefact to simplify the data access from the cluster. Kafka utilises the binary protocol instead of HTTP, whereas most services depend on HTTP for network connection. 
...
Next, the major system to be implemented is Data Lichen. Data Lichen required a new frontend to be developed from the scratch. A template engine for Node.js called EJS is chosen for this purpose. This template engine was selected for its ability to simplify development and speed up the operations of this trial. 

The goal is to create a simple user interface for Data Lichen that successfully showcases the framework's potential design and capability, rather than building a comprehensive frontend. Various teams may opt to adopt Data Lichen in diverse manners. Data Lichen must be developed from the scratch because there is no existing open-source technology that offers the necessary features.
...
Istio was chosen because it has already been selected and deployed on the cluster. Nevertheless, automatically injecting policies into services inside a particular service mesh is a complex operation. This was done extending the Envoy sidecar with an external authorizer API to implement inbound and outbound policies. 
...
Two important data sources studied and selected for creating these domains are the Yelp Academic Dataset \cite{YelpDataset} and the data from the U.S. Climate Resilience Toolkit's Climate Explorer \cite{ClimateExplorer}. This decision is based on the extensive and detailed data provided by these sources, their accessibility, and their ability to generate valuable analytical insights when utilised together.
...
The climatic Explorer dataset, which provides a detailed record of past and predicted climatic factors in the United States, is selected as an appropriate option for studying possible connections between weather trends and restaurant evaluations. Although options like Visual Crossing Weather Data Services \cite{visualcrossing} have been examined, Climate Explorer's extensive global coverage and historical data are preferred.
